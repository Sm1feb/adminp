	...
	<aside>
		...
		<div class="col-md-10 main-section">
			<section>
				...
				<div>
				@if(@isset($findrec[0]))
                <form action="{{url('k_editdata/'.$findrec[0]->id)}}" method="post" enctype="multipart/form-data">
            @else
				...
				{{csrf_field()}}
					...
					<tr>
							...
							<td>
                            <select name="category" value="{{isset($findrec[0]->categoryname) ? $findrec[0]->categoryname:""}}">
                            @foreach($date as $row)
								
                               
                                    <option  class="hloo" value="{{$row->categoryname}}">{{$row->categoryname}}</option>
                        ...
                        <tr>
							...
							<td>
								<input name="description" value="{{isset($findrec[0]->description) ? $findrec[0]->description:""}}" style="width:200px","height:70px"; ></textarea>
							</td>
						</tr>
                        <tr>
							<td style="width:250px; text-align:right">Product Price<span style="color:red">*</span></td>
							<td>
								<input type="text" name="price" style="width:200px" value="{{isset($findrec[0]->price) ? $findrec[0]->price:""}}">
							</td>
						</tr>
                        <tr>
							<td style="width:250px; text-align:right">Product Image<span style="color:red">*</span></td>
							<td>
								<input type="file" name="image" value="{{isset($findrec[0]->image) ? $findrec[0]->image:""}}">
							</td>
						</tr>
