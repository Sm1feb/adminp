	...
	<aside>
		...
		<div class="col-md-10 main-section">
			<section>
				...
				@if(@isset($findrec[0]))
                <form action="{{url('p_editdata/'.$findrec[0]->id)}}" method="post">
            @else
            @endif
				<form action="{{url('categoryname')}}" method="post">
				{{csrf_field()}}
					<table class="addpage-table">
						...
						<tr>
							...
							<td>
								<input type="text" name="catname" style="width:200px" value="{{isset($findrec[0]->categoryname) ? $findrec[0]->categoryname:""}}">
							</td>
						</tr>
